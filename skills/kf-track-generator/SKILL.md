---
name: kf-track-generator
description: Research the codebase and project context to generate kiloforge tracks from a user prompt. Splits large work into multiple tracks (including BE/FE splits). Merges track artifacts to main. Part of the track generation => approval => push to worker workflow.
metadata:
  argument-hint: "<prompt describing the desired feature/change>"
---

# Kiloforge Track Generator
...
```
ACTIVE ROLE: kf-track-generator — skill at ~/.claude/skills/kf-track-generator/SKILL.md
```
...
_Generated by kf-track-generator from prompt: "{original prompt}"_
```
