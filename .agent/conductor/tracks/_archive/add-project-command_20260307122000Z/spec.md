# Specification: Implement 'kf add' Command for Project Registration
...
Implement `kf add <repo-path>` to register a project with the global Gitea instance. Records the project's origin remote URL in the project registry so kiloforge can bridge local Gitea work back to the real remote on demand. Creates a Gitea repo, adds a `gitea` git remote, pushes code, registers a webhook, and tracks the project in a new `projects.json` registry. Also adds `kf projects` to list registered projects.

## Context

After `kf init`, the user has a running Gitea instance but no projects registered. The `add` command bridges a local repo into the kiloforge system. The key architectural insight is that kiloforge works locally against Gitea but needs to push changes back to the user's real remote (e.g., GitHub) when asked. To support this, the project registry stores the origin remote URL captured at registration time.
...
- **`internal/cli/init.go`** — Line 123: `fmt.Println("Next: use 'kf add' to register a project (coming soon).")` — this placeholder gets fulfilled.
...
- [ ] `kf add <repo-path>` registers a project with the global Gitea instance
- [ ] `kf add` with no args uses the current working directory
...
- [ ] Project registry (`projects.json`) created/updated in `~/.kiloforge/`
...
- [ ] Project data directory created: `~/.kiloforge/projects/<slug>/logs/`
- [ ] `kf projects` lists all registered projects with slug, path, origin, and status
- [ ] Idempotent: running `add` on an already-registered project prints a message and exits cleanly
- [ ] Error if Gitea is not running (directs user to run `kf init` first)
...
- `kf remove` command (future track)
- `kf push` / bridge command to push back to origin (future track — registry stores origin URL to enable this)
...
```json
{
  ...
  "projects": {
    "kiloforge": {
      ...
      "origin_remote": "git@github.com:user/kiloforge.git",
...
The `origin_remote` field stores the URL of the project's `origin` git remote at the time of registration. This enables future bridging: work locally against Gitea, then `kf push` syncs back to the real remote.
...
14. Create project data dir: ~/.kiloforge/projects/<slug>/logs/
...
_Generated by conductor-track-generator from prompt: "Add a track for adding a repo to the system via 'kf add' with origin remote tracking for bridging"_
