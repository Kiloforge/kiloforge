...
- [x] Task 1.1: Update `backend/go.mod` module line from `crelay` to `kiloforge`
- [x] Task 1.2: Bulk rename all Go import paths from `"crelay/` to `"kiloforge/` across all .go files
- [x] Task 1.3: Rename directory `backend/cmd/crelay/` to `backend/cmd/kf/`
...
- [x] Task 2.1: Update cobra root command `Use` from `"crelay"` to `"kf"` in `root.go`
- [x] Task 2.2: Update cobra root command `Long` description to reference "kiloforge"
- [x] Task 2.3: Update any subcommand help text that references "crelay" (e.g., init.go, add.go help strings)
- [x] Task 2.4: Verify: `./bin/kf --help` shows correct branding

## Phase 3: Environment Variables and Config Paths

- [x] Task 3.1: Rename env var prefix from `CRELAY_` to `KF_` in `env_adapter.go`
- [x] Task 3.2: Update default data directory from `~/.crelay` to `~/.kiloforge` in `defaults.go`
- [x] Task 3.3: Update env var references in `env_adapter_test.go`
- [x] Task 3.4: Update env var references in `resolve_test.go`
- [x] Task 3.5: Search for any other `CRELAY_` references in Go code and update
- [x] Task 3.6: Verify: `make test` passes for config package

## Phase 4: API Specifications

- [x] Task 4.1: Update OpenAPI spec title from "Crelay API" to "Kiloforge API" in `backend/api/openapi.yaml`
...
- [x] Task 5.2: Update heading in `frontend/src/App.tsx` from "crelay" to "kiloforge"
- [x] Task 5.3: Update help text in `frontend/src/pages/OverviewPage.tsx` — change `crelay add` to `kf add`
- [x] Task 5.4: Search for any other "crelay" references in frontend/ and update
...
- [x] Task 7.4: Update `.agent/conductor/product-guidelines.md` if it references "crelay"
- [x] Task 7.5: Update skill files that reference `CRELAY_RELAY_URL` to `KF_RELAY_URL`
...
- [x] Task 8.4: Grep for any remaining "crelay" references (case-insensitive) — verify only historical track specs remain
