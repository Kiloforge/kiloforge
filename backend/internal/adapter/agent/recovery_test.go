...
func TestDetectStale_MarksDeadRunningAsSuspended(t *testing.T) {
	store := &testutil.MockAgentStore{
		AgentData: []domain.AgentInfo{
			// PID 999999 should not exist — stale running agent
			{ID: "stale-1", Status: "running", PID: 999999, SessionID: "sess-1", WorktreeDir: os.TempDir()},
			// PID 0 should not be checked
			{ID: "no-pid", Status: "running", PID: 0, SessionID: "sess-2", WorktreeDir: os.TempDir()},
		},
	}
	starter := &mockProcessStarter{}
	rm := NewRecoveryManager(store, starter)
	count := rm.DetectStale()

	if count != 1 {
		t.Errorf("expected 1 stale agent, got %d", count)
	}

	// stale-1 should be suspended
	a1, _ := store.FindAgent("stale-1")
	if a1.Status != "suspended" {
		t.Errorf("expected stale-1 suspended, got %s", a1.Status)
	}

	// no-pid should remain running (PID 0 is not checked)
	a2, _ := store.FindAgent("no-pid")
	if a2.Status != "running" {
		t.Errorf("expected no-pid still running, got %s", a2.Status)
	}

	// No resume calls should have been made
	if len(starter.calls) != 0 {
		t.Errorf("expected no start calls, got %d", len(starter.calls))
	}
}

func TestDetectStale_IgnoresAliveProcesses(t *testing.T) {
	// Use current PID — it's definitely alive
	store := &testutil.MockAgentStore{
		AgentData: []domain.AgentInfo{
			{ID: "alive-1", Status: "running", PID: os.Getpid(), SessionID: "sess-1", WorktreeDir: os.TempDir()},
		},
	}
	starter := &mockProcessStarter{}
	rm := NewRecoveryManager(store, starter)
	...
	a, _ := store.FindAgent("alive-1")
	if a.Status != "running" {
		t.Errorf("expected alive-1 still running, got %s", a.Status)
	}
}

func TestDetectStale_ChecksWaitingAndSuspending(t *testing.T) {
	store := &testutil.MockAgentStore{
		AgentData: []domain.AgentInfo{
			{ID: "w-1", Status: "waiting", PID: 999999, SessionID: "s1", WorktreeDir: os.TempDir()},
			{ID: "s-1", Status: "suspending", PID: 999998, SessionID: "s2", WorktreeDir: os.TempDir()},
		},
	}
	starter := &mockProcessStarter{}
	rm := NewRecoveryManager(store, starter)
	...
	if count != 2 {
		t.Errorf("expected 2 stale agents, got %d", count)
	}
}
